...
export function PerspectiveCarousel({
  ...
  return (
    ...
    >
      <div
        className={`absolute inset-0 overflow-hidden ${viewportClassName || ""}`}
        ...
        >
          {items.map((item, index) => {
            ...
            return (
              ...
              >
                <motion.div
                  className={`flex w-full flex-col items-center gap-3 will-change-transform ${slideClassName || ""}`}
                  animate={{
                ...
                >
                  ...
                  >
                    ...
                    <img
                      ...
                      draggable={false}
                      className={`h-full w-full select-none rounded-lg object-cover shadow-xl ${imageClassName || ""}`}
                    />
                  </button>

                  <motion.p
                    className={`whitespace-nowrap text-sm ${labelClassName || ""}`}
                    animate={{
      ...
      {showControls && (
        <div
          className={`absolute inset-x-4 bottom-5 z-10 mx-auto flex w-fit items-center justify-center gap-3 rounded-full border border-neutral-300/80 bg-neutral-200/70 px-2 text-neutral-700 shadow-sm backdrop-blur-sm dark:border-white/10 dark:bg-neutral-900/70 dark:text-neutral-100 ${controlsClassName || ""}`}
        >
          <button
            type="button"
            ...
            <div className="flex items-center justify-center gap-2">
              {items.map((item, index) => (
                <button
                  ...
                  className={`h-2 rounded-full bg-current transition-[width,opacity] duration-300 ${currentIndex === index ? "w-7 opacity-100" : "w-2 opacity-30"}`}
                  onClick={() => selectSlide(index)}
                />
              ))}
            </div>
